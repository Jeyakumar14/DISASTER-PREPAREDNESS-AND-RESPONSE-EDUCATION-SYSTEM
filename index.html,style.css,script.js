...
<head>
  ...
  <title>Sparh-Up — SIH Prototype</title>
...
<body>
  ...
  <section id="page-map" class="page">
      ...
      <div style="display:flex; gap:12px; justify-content:center; margin-bottom:12px;">
        ...
        <button class="btn" id="panicBtn" style="background:var(--danger)">🚨 Panic Alert</button>
      </div>
  ...
  <footer><small>Prototype for SIH 2025 — Disaster Education & Response System</small></footer>
  ...
  (function(){
    ...
    function renderModules(){
      ...
      MODULES.forEach(m=>{
        ...
        div.innerHTML = <h4>${m.title}</h4><p style="font-size:14px; color:var(--muted)">${m.summary}</p><div style="display:flex;justify-content:space-between;align-items:center;margin-top:8px"><button class='btn' data-id='${m.id}'>Open</button><div style='color:${prog[m.id]?"var(--success)":"var(--muted)"}'>${prog[m.id]? '✔ Completed':'Not started'}</div></div>;
        out.appendChild(div);
      });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Panic button — demo: log details and show toast
  panicBtn.addEventListener('click', () => {
    ...
    navigator.geolocation.getCurrentPosition(position => {
      ...
      toast('🚨 Panic alert sent (demo)', 'danger', 4000);
    }, err => {
